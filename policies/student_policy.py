...
from graspbench.config import CONTAINER_SPEC_BY_NAME, HOME_Q, OBJECT_SPEC_BY_NAME
...
# At HOME_Q, the arm's own body sits inside the overhead camera's view of the
# tabletop. Confirmed by debugging a live failure: Grounding DINO's box for
# "yellow square tray" either engulfed most of the frame (tabletop + part of
# the arm) or, in an uncluttered re-check, scored a correct small box at
# 0.312 -- just above the 0.30 threshold -- yet the real HTTP pipeline still
# returned zero candidates, meaning something about the arm's presence in
# frame was tipping a borderline detection the wrong way. Swinging the arm
# out of frame before the very first detection removes the arm as a
# variable entirely, which a tighter score threshold or box-size filter
# cannot: those only react to a bad detection, they cannot prevent the
# camera's view from being obstructed/cluttered in the first place. Only
# the first perception call is guarded this way (not verify()/retries) to
# keep the added motion cost to one clearing move per episode.
# Single-joint move: only joint2 (shoulder pitch) rotates, from HOME_Q's
# -0.45 to -1.5, folding the whole arm back over its own base. joint1 (base
# yaw) and every other joint are untouched -- the arm retracts straight
# back rather than swinging sideways off the table. After detection
# completes, the same single joint retraces its path back to HOME_Q
# (see _restore_pose) before the pick sequence starts.
CLEAR_VIEW_Q = HOME_Q.copy()
CLEAR_VIEW_Q[1] = -1.3
CLEAR_VIEW_TOLERANCE = 0.03
...
class StudentPolicy:
    ...
    def act(self, observation: Observation) -> PolicyDecision:
        ...
        if self.actions is None:
            if self.stage == "clear_view":
                return self._move_to_clear_view(observation)
            try:
        ...
        if self.stage == "restore_pose":
            # Undo the clear_view retraction by the same path: swing joint2
            # back to its original HOME_Q angle before starting the actual
            # pick sequence, so the arm returns to its normal working pose
            # rather than starting every episode folded back over its base.
            return self._restore_pose(observation)
    ...
    def _move_to_clear_view(self, observation: Observation) -> PolicyDecision:
        # Pure joint-space move toward CLEAR_VIEW_Q: the goal is only to get
        # the arm's body out of the overhead camera's frame before the first
        # detection, not to reach any particular Cartesian pose, so this
        # does not go through _move_to's Cartesian-waypoint IK path.
        next_joint = move_toward(observation.joint_position, CLEAR_VIEW_Q)
        joint_error = float(np.max(np.abs(observation.joint_position - CLEAR_VIEW_Q)))
        if joint_error < CLEAR_VIEW_TOLERANCE:
            self.stage = "init"
        return PolicyDecision(
            command=JointPositionCommand(next_joint, 1.0),
            stage="clear_view",
            rationale=f"Swinging the arm out of the overhead camera's view before first detection; joint_error={joint_error:.3f}rad.",
        )

    def _restore_pose(self, observation: Observation) -> PolicyDecision:
        # Retrace the same single-joint path back to HOME_Q now that
        # detection is done, instead of starting the pick sequence from the
        # folded-back clear_view pose.
        next_joint = move_toward(observation.joint_position, HOME_Q)
        joint_error = float(np.max(np.abs(observation.joint_position - HOME_Q)))
        if joint_error < CLEAR_VIEW_TOLERANCE:
            self.stage = "rise_pick"
        return PolicyDecision(
            command=JointPositionCommand(next_joint, 1.0),
            stage="restore_pose",
            rationale=f"Retracting back to the home pose after detection; joint_error={joint_error:.3f}rad.",
        )
