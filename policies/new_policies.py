    ...
    def reset(self, task: dict, model: mujoco.MjModel) -> None:
            ...
            if len(self.action_plan) > 1:
                ...
                if self.target_id in {"apple", "orange"}:
                    for other, _ in peers:
                        if other != self.target_id and other in self.completed_positions:
                            ...
                            known = self.completed_positions[other]
                            separation = (self.observed_radii.get(other, OBJECT_SPEC_BY_NAME[other].half_height)
                                          + self.observed_radii.get(self.target_id, target_spec.half_height) - 0.003)
        ...
        def activate_action(detections):
            ...
            self.bottle_tilt = (self.target_id == "mustard_bottle"
                                and np.linalg.norm(self.target_world[:2]) > 0.68)
        ...
        self.policy_revision = "task123_sort_v63"
        ...
        try:
            ...
            if self.stage == "reacquire":
                ...
                if recovered is not None:
                    ...
                    self.bottle_tilt = self.target_id == "mustard_bottle" and radius > 0.68
            ...
            if self.stage == "release":
                if observation.time - self.stage_started_s >= self.release_duration_s:
